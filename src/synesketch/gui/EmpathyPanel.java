...
public class EmpathyPanel extends JPanel {
	 ...
	 * @throws Exception
	 */
	public EmpathyPanel(int appletSize, String artType,	String synesthetiatorType) throws Exception {
		
		Class<?> appletDefinition = Class.forName(appletClassNamePrefix + artType);
		
		Constructor<?> appletConstructor = appletDefinition.getConstructor(int.class);
		
		embed = (PApplet) appletConstructor.newInstance(appletSize);
		
		Class<?> syneDefinition = Class.forName(synesthetiatorType);
		
		Constructor<?> syneConstructor = syneDefinition.getConstructor(PApplet.class);
