...
public class EmotionalState extends SynesketchState {
	...
	/**
	 * Class constuctor which sets the text and map
	 * 
	 * @param text
	 ...
	 *            {@link String} representing the text
	 * @param emotions
	 *            {@link Map} containing all of the specific Ekman emotional weights, defined by the semantic annotation
	 * 
	 */
	public EmotionalState(String text, Map<String, Double> emotionAnnotations) {		
		super(text);
		
		emotions = new TreeSet<Emotion>();
		double positiveEmotionWeight = 0;
		double maxNegativeEmotionWeight = 0;
		
		
		
		for (Entry<String, Double> emotionAnnotation : emotionAnnotations.entrySet()) {
			...
			String[] uriEmotions = Emotion.URI_EMOTIONS;
			
			/*if (uriEmotion.equals(Emotion.URI_EMOTIONS[Emotion.HAPPINESS])) {
				emotions.add(new Emotion(emotionalWeight, Emotion.HAPPINESS));
				positiveEmotionWeight = emotionalWeight;
			} else if (uriEmotion.equals(Emotion.URI_EMOTIONS[Emotion.SADNESS])) {
				emotions.add(new Emotion(emotionalWeight, Emotion.SADNESS));
				if (emotionalWeight > maxNegativeEmotionWeight) maxNegativeEmotionWeight = emotionalWeight;
			} else if (uriEmotion.equals(Emotion.URI_EMOTIONS[Emotion.ANGER])) {
				emotions.add(new Emotion(emotionalWeight, Emotion.ANGER));
				if (emotionalWeight > maxNegativeEmotionWeight) maxNegativeEmotionWeight = emotionalWeight;
			} else if (uriEmotion.equals(Emotion.URI_EMOTIONS[Emotion.FEAR])) {
				emotions.add(new Emotion(emotionalWeight, Emotion.FEAR));
				if (emotionalWeight > maxNegativeEmotionWeight) maxNegativeEmotionWeight = emotionalWeight;
			} else if (uriEmotion.equals(Emotion.URI_EMOTIONS[Emotion.DISGUST])) {
				emotions.add(new Emotion(emotionalWeight, Emotion.DISGUST));
				if (emotionalWeight > maxNegativeEmotionWeight) maxNegativeEmotionWeight = emotionalWeight;
			} else if (uriEmotion.equals(Emotion.URI_EMOTIONS[Emotion.SURPRISE])) {
				emotions.add(new Emotion(emotionalWeight, Emotion.SURPRISE));				
			} */
			
			for (int i = 0; i < uriEmotions.length; i++) {
				if (uriEmotion.equals(uriEmotions[i])) {
					
					emotions.add(new Emotion(emotionalWeight, i));
					
					if (i == Emotion.HAPPINESS) {
						positiveEmotionWeight = emotionalWeight;						
					} else if (Emotion.isNegative(i)) {
						if (emotionalWeight > maxNegativeEmotionWeight) maxNegativeEmotionWeight = emotionalWeight;
					}
				}
			}
		...
		if (positiveEmotionWeight > maxNegativeEmotionWeight) {
			valence = 1;
		} else if (positiveEmotionWeight < maxNegativeEmotionWeight) {
			valence = -1;
		} else {
			valence = 0;
		}
		...
		if (!emotions.isEmpty()) {
			generalWeight = emotions.first().getWeight();			
		}
	...
	public SortedSet<Emotion> getEmotionSet() {
		return emotions;
	}
